...
namespace MoreMountains.Feedbacks
{
	...
	public abstract class MMF_Feedback
	{
		...
		[Tooltip("a number of timing-related values (delay, repeat, etc)")] [SerializeReference]
		public MMFeedbackTiming Timing = null;
		[FoldoutGroup("Setting")]
		...
		public virtual bool InCooldown => (TimingProp.CooldownDuration > 0f) &&
		                                  (FeedbackTime - _lastPlayTimestamp < TimingProp.CooldownDuration);
		...
		{
			float result = TimingProp.ConstantIntensity ? 1f : intensity;
		...
		public virtual TimescaleModes ComputedTimescaleMode
		{
			get
			{
				...
				return TimingProp.TimescaleMode;
			}
		}
		...
		public virtual bool InScaledTimescaleMode
		{
			get
			{
				...
				return (TimingProp.TimescaleMode == TimescaleModes.Scaled);
			}
		}
		...
		public virtual float FeedbackTime
		{
			get
			{
				...
				if (TimingProp.UseScriptDrivenTimescale)
				{
					return TimingProp.ScriptDrivenTime;
				}
				...
				if (TimingProp.TimescaleMode == TimescaleModes.Scaled)
				{
					return Time.time;
				}
				else
		...
		public virtual float FeedbackDeltaTime
		{
			get
			{
				if (TimingProp.UseScriptDrivenTimescale)
				{
					return TimingProp.ScriptDrivenDeltaTime;
				}
		...
		public virtual void Initialization(MMF_Player owner, int index)
		{
			if (TimingProp == null)
			{
				TimingProp = new MMFeedbackTiming();
			}
			...
			_initialized = true;
			Owner = owner;
			_playsLeft = TimingProp.NumberOfRepeats + 1;
			_channelData = new MMChannelData(ChannelMode, Channel, MMChannelDefinition);
			AutomateTargetAcquisitionInternal();
			SetInitialDelay(TimingProp.InitialDelay);
			SetDelayBetweenRepeats(TimingProp.DelayBetweenRepeats);
			SetSequence(TimingProp.Sequence);
		...
		{
			yield return WaitFor(TimingProp.InitialDelay);
			RegularPlay(position, feedbacksIntensity);
			_lastPlayTimestamp = FeedbackTime;
		}
		...
		protected virtual void RegularPlay(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			if (TimingProp.UseIntensityInterval)
			{
				if ((feedbacksIntensity < TimingProp.IntensityIntervalMin) ||
				    (feedbacksIntensity >= TimingProp.IntensityIntervalMax))
				{
					return;
				}
			}

			if (TimingProp.RepeatForever)
			{
				_infinitePlayCoroutine = Owner.StartCoroutine(InfinitePlay(position, feedbacksIntensity));
				return;
			}

			if (TimingProp.NumberOfRepeats > 0)
			{
				_repeatedPlayCoroutine = Owner.StartCoroutine(RepeatedPlay(position, feedbacksIntensity));
				return;
			}

			if (TimingProp.Sequence == null)
			{
		...
		protected virtual IEnumerator InfinitePlay(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			while (true)
			{
				if (TimingProp.Sequence == null)
				{
					CustomPlayFeedback(position, feedbacksIntensity);
					_lastPlayTimestamp = FeedbackTime;
					yield return WaitFor(TimingProp.DelayBetweenRepeats);
				}
				else
				{
					_sequenceCoroutine = Owner.StartCoroutine(SequenceCoroutine(position, feedbacksIntensity));

					float delay = ApplyTimeMultiplier(TimingProp.DelayBetweenRepeats) + TimingProp.Sequence.Length;
		...
		protected virtual IEnumerator SequenceCoroutine(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			if (TimingProp.Quantized)
			{
				while (CurrentSequenceIndex < TimingProp.Sequence.QuantizedSequence[0].Line.Count)
				{
					_beatInterval = 60f / TimingProp.TargetBPM;
					...
					if ((FeedbackTime - LastBeatTimestamp >= _beatInterval) || (LastBeatTimestamp == 0f))
					{
						...
						for (int i = 0; i < TimingProp.Sequence.SequenceTracks.Count; i++)
						{
							if (TimingProp.Sequence.QuantizedSequence[i].Line[CurrentSequenceIndex].ID == TimingProp.TrackID)
							{
								CustomPlayFeedback(position, feedbacksIntensity);
							}
						}
			...
			{
				while (FeedbackTime - timeStartedAt < TimingProp.Sequence.Length)
				{
					foreach (MMSequenceNote item in TimingProp.Sequence.OriginalSequence.Line)
					{
						if ((item.ID == TimingProp.TrackID) && (item.Timestamp >= lastFrame) &&
		...
		{
			TimingProp.Sequence = newSequence;
			if (TimingProp.Sequence != null)
			{
				for (int i = 0; i < TimingProp.Sequence.SequenceTracks.Count; i++)
				{
					if (TimingProp.Sequence.SequenceTracks[i].ID == TimingProp.TrackID)
					{
						_sequenceTrackID = i;
					}
				}
		...
		public virtual void Stop(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			_lastPlayTimestamp = -1f;
			_playsLeft = TimingProp.NumberOfRepeats + 1;
			if (TimingProp.InterruptsOnStop)
			{
		...
		{
			TimingProp.DelayBetweenRepeats = delay;
		}
		...
		{
			if ((TimingProp != null) && (!TimingProp.ContributeToTotalDuration))
			{
				_totalDuration = 0f;
				return;
			}
			...
			if (TimingProp.InitialDelay != 0)
			{
				totalTime += ApplyTimeMultiplier(TimingProp.InitialDelay);
			}
			...
			if (TimingProp.NumberOfRepeats != 0)
			{
				float delayBetweenRepeats = ApplyTimeMultiplier(TimingProp.DelayBetweenRepeats);

				totalTime += (TimingProp.NumberOfRepeats * delayBetweenRepeats);
			}
		...
		public virtual bool NormalPlayDirection
		{
			get
			{
				switch (TimingProp.PlayDirection)
				{
		...
		public virtual bool ShouldPlayInThisSequenceDirection
		{
			get
			{
				...
				switch (TimingProp.MMFeedbacksDirectionCondition)
				{
		...
		protected MMFeedbackTiming timingRuntime;

		public static MMFeedbackTiming DefaultTiming = new MMFeedbackTiming();
		/// a number of timing-related values (delay, repeat, etc)
		public MMFeedbackTiming TimingProp
		{
			get => timingRuntime ?? (Timing != null ? timingRuntime = Timing : timingRuntime = (Application.isPlaying ? new MMFeedbackTiming() : DefaultTiming) );
			set => Timing = value;
		}

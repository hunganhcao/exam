	...
	public class MMF_Player : MMFeedbacks
	{
		...
		protected override IEnumerator PausedFeedbacksCo(Vector3 position, float feedbacksIntensity)
		{
			...
			while ((i >= 0) && (i < count))
			{
				...
				if (FeedbacksList[i].Active)
				{
					if ((FeedbacksList[i].Pause == null) && (FeedbacksList[i].ShouldPlayInThisSequenceDirection) && (!FeedbacksList[i].TimingProp.ExcludeFromHoldingPauses))
					{
		...
		public new MMF_Feedback AddFeedback(System.Type feedbackType, bool add = true)
		{
			...
			if(Application.isPlaying)
				newFeedback.TimingProp = new MMFeedbackTiming();
			newFeedback.UniqueID = Guid.NewGuid().GetHashCode();
			if (add)
			{
				FeedbacksList.Add(newFeedback);	
		...
		public override bool HasFeedbackStillPlaying()
		{
			...
			for (int i = 0; i < count; i++)
			{
				if ((FeedbacksList[i].IsPlaying 
				     && !FeedbacksList[i].TimingProp.ExcludeFromHoldingPauses)
				    || FeedbacksList[i].TimingProp.RepeatForever)
				{
					return true;
				}
			}
		...
		{
			if (feedback.TimingProp.MMFeedbacksDirectionCondition == MMFeedbackTiming.MMFeedbacksDirectionConditions.Always)
			{
				return true;
			}
			else if (((Direction == Directions.TopToBottom) && (feedback.TimingProp.MMFeedbacksDirectionCondition == MMFeedbackTiming.MMFeedbacksDirectionConditions.OnlyWhenForwards))
			         || ((Direction == Directions.BottomToTop) && (feedback.TimingProp.MMFeedbacksDirectionCondition == MMFeedbackTiming.MMFeedbacksDirectionConditions.OnlyWhenBackwards)))
			{
				return true;
			}
			return false;
